...
import { Space, Button, Input, Modal, message, Tooltip, Popconfirm, List, Avatar, DatePicker } from "antd";
import { CheckCircleTwoTone, StopTwoTone, CalendarTwoTone, ProfileTwoTone } from "@ant-design/icons";
import { getPaymentLDList, submitLDToCW, rejectOne, logsOne, getPaymentDetailById } from "@/restApi/payment";
...
const Role = () => {
  ...
  const handleSubmitToCW = async () => {
    ...
    const res = await getPaymentLDList(page, pageSize, searchValue, supplierId, projectState, userName, projectNum, date);
    setData(res);
  ...
  return (
    <div className="p-2">
      ...
      {!!detail && <PaymentSubmitModal data={detail} onConfirm={handleSubmitToCW} onClose={() => setDetail(undefined)} />}
